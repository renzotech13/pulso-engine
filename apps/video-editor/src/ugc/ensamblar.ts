...
export async function componerElementos(o: ComponerOpts): Promise<string> {
  ...
  if (cadena.length === 0) {
    // Sin capas (solo subtítulos): igual hay que llevar el video al formato pedido (1080x1920 o recorte 4:5).
    await ff(["-i", W("base.mp4"), "-i", fuente, "-map", "0:v", "-map", "1:a", "-c:v", "copy", "-c:a", "aac", "-b:a", "192k", "-shortest", conElementos]);
  } else {
  ...
  if (el.subtitulos && o.guionSubtitulos) {
    ...
    await aplicarSubtitulos({
      ...
      palabrasPorFila: el.subtitulos.palabrasPorFila,
      colorResalte: el.subtitulos.colorResalte,
    });
    return salidaSub;
  }
  return conElementos;
