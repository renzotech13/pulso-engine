...
export const inputClass =
  "w-full rounded-lg border border-ink-700 bg-ink-900 px-3 py-2 text-sm text-neutral-100 placeholder:text-neutral-600 focus:border-pulso-accent focus:outline-none focus:ring-2 focus:ring-pulso-accent/30 disabled:opacity-60";
  ...
  return (
    <div className={className}>
      <label htmlFor={id} className={labelClass}>
        {label}
        {required && <span className="ml-0.5 text-status-pink">*</span>}
      </label>
      {children}
      {error ? (
        <p role="alert" className="mt-1 text-xs text-status-pink">
          {error}
        </p>
      ) : hint ? (
