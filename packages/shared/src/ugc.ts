...
export const ugcElementosSchema = z.object({
  ...
  subtitulos: z
    .object({
      ...
      /** Palabras por fila (el estilo trae 3; menos = subtítulos más cortos). */
      palabrasPorFila: z.number().int().min(1).max(4).optional(),
      /** Color de la palabra activa (el estilo trae celeste/dorado); vacío = el del estilo. */
      colorResalte: z.string().regex(/^#[0-9a-fA-F]{6}$/).optional(),
    })
    .nullable()
    .default(null),
  /**
